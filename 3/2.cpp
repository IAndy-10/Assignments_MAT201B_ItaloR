...
struct AlloApp : DistributedAppWithState<WorldState> {
  ...
  Parameter lineOpacity{"/lineOpacity", "", 1.0, 0.0, 1.0};
  Parameter lineWidth{"/lineWidth", "", 1.0, 1.0, 10.0};
  ...
  void onCreate() override {
    ...
    pointShader.compile(slurp(dir + "/point-vertex.glsl"),
                        slurp(dir + "/point-fragment.glsl"),
                        slurp(dir + "/point-geometry.glsl"));
    ...
    lineShader.compile(
      R"(#version 400
         ...
         void main() { gl_Position = al_ProjectionMatrix * al_ModelViewMatrix * vec4(vertexPosition, 1.0); })",
      slurp(dir + "/point-fragment-2.glsl"),
      ""
    );
  ...
  void onAnimate(double dt) override {
    ...
    // Sync particles and camera to WorldState for distribution
    for (int i = 0; i < N; i++) {
      state().position[i] = particles.vertices()[i];
      state().color[i] = particles.colors()[i];
    }
    state().camera = nav();
  }
  ...
  void onDraw(Graphics &g) override {
    ...
    g.clearColor(0.5, 0.5, 0.5); // Gray background
    ...
    if (loveLines){
      g.shader(lineShader);
      g.shader().uniform("lineColor", Vec4f(0.0, 0.0, 1.0, lineOpacity));
      g.lineWidth(lineWidth);
      Mesh lines;
      lines.primitive(Mesh::LINES);
      for (int i = 0; i < velocity.size(); i++){
        lines.vertex(particles.vertices()[i]);
        lines.vertex(particles.vertices()[lovers[i]]);
      }
      g.draw(lines);
    }
    ...
    if (!isPrimary()) {
      for (int i = 0; i < N; i++) {
        ...
        particles.colors()[i] = state().color[i];
        // fadeQuad.vertices()[i] = state().position[i];
        // fadeQuad.colors()[i] = state().color[i];
      }
    }
  }
};
